...
class TodosCreateView extends StatefulWidget {
  const TodosCreateView({super.key});

  @override
  State<TodosCreateView> createState() => _TodosCreateViewState();
}

class _TodosCreateViewState extends State<TodosCreateView> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<TodosBloc, TodosState>(
      listenWhen: (previous, current) =>
          previous.todosCreateStatus != current.todosCreateStatus,
      listener: (context, state) {
        if (state.todosCreateStatus is StatusLoading) {
          CustomLoading(context: context).show();
        } else if (state.todosCreateStatus is StatusSuccess) {
        ...
        } else if (state.todosCreateStatus is StatusFailure) {
          CustomLoading.dissmis(context);
          CustomMessage(
            context: context,
                  ...
                  PrimaryButton(
                    ...
                    onPressed: () {
                      if (formKey.currentState!.validate()) {
                        ...
                        context
                            .read<TodosBloc>()
                            .add(TodosCreateEvent(todosModel: todos));
                      }
                    },
                    title: 'Save',
                  ),
